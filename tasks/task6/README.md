...
## 1. Что такое "ложная разделяемость" (false sharing)

False sharing возникает, когда несколько потоков обращаются к разным переменным, которые находятся на одной кэш-линии CPU. Из-за этого кэш-линия постоянно обновляется между ядрами, что снижает производительность.

## 2. Почему для `SCHED_FIFO` и `sched_setaffinity` нужны права `sudo`

Эти функции изменяют приоритет планировщика и привязку к ядрам, что может повлиять на всю систему. Без root ограничен максимальный приоритет. Можно использовать `CAP_SYS_NICE` через `setcap` для конкретного исполняемого файла.

## 3. Как real-time задача выполняет системные вызовы на изолированном ядре

Даже если ядро изолировано для планирования задач ОС, системные вызовы могут выполняться через ядра, на которых работают ядра ОС, через межъядерное переключение (inter-processor interrupts) и планировщик ядра.

## 4. Почему привязка к одному ядру может ухудшить производительность

Ограничение задачи одним ядром может привести к недостаточному использованию других ядер, снижению параллелизма, очередям готовности, а также увеличению задержек из-за блокировок и ожидания ресурсов.
