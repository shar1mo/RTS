...
### 1. Отличие символьных и блочных устройств

- **Символьные устройства**: данные обрабатываются потоками по одному символу за раз, без кэширования и буферизации.  
  **Примеры**: `/dev/tty` (терминал), `/dev/input/event0` (клавиатура).  
- **Блочные устройства**: данные обрабатываются блоками фиксированного размера с возможностью кэширования и случайного доступа.  
  **Примеры**: `/dev/sda` (жёсткий диск), `/dev/mmcblk0` (SD-карта).  

---

### 2. Почему `poll()` предпочтительнее busy-wait

- `poll()` позволяет **ожидать события от нескольких источников** без постоянного цикла опроса CPU.  
- **Сценарий критичности**: многопоточный сервер, который ждёт ввода с клавиатуры, данных из сети и таймеров одновременно. Busy-wait будет сильно нагружать процессор, в то время как `poll()` блокирует процесс до события.

---

### 3. Может ли `read()` для символьного устройства блокировать?

- Да, `read()` может блокировать процесс до появления данных (например, от клавиатуры или устройства `/dev/input/`).  
- **Как избежать**: использовать неблокирующий режим (O_NONBLOCK) или опрашивать с помощью `select()`/`poll()`.

---

### 4. Почему для работы с `ioctl` и `/dev/input/` требуются Linux-заголовки

- Структуры и константы (например, `struct input_event`) определены только в Linux (`<linux/input.h>`).  
- **Переносимость**: такой код **не переносим** на другие POSIX-системы (macOS, FreeBSD), так как устройства `/dev/input/` и ioctl-определения отличаются.
