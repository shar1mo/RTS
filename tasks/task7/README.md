...
## 1. Почему использование сигналов от таймера предпочтительнее, чем `sleep()`

Сигналы таймера обеспечивают точное периодическое срабатывание, независимо от работы процесса. `sleep()` блокирует поток на фиксированное время, что может привести к накоплению задержек и пропуску событий, особенно если обработка состояния занимает значительное время.

## 2. Где может возникнуть состояние гонки и как мьютекс помогает

Состояние гонки возможно между потоком ввода и потоком контроллера при доступе к общим данным (например, флаг запроса пешехода). Мьютекс гарантирует, что только один поток одновременно изменяет или читает эти данные, предотвращая неконсистентное состояние.

## 3. Изменения архитектуры для управления GPIO

Нужно заменить вывод в консоль на функции работы с GPIO (например, через `wiringPi` или `libgpiod`). Поток контроллера будет писать значения в пины, а ввод с кнопок — считывать через GPIO, сохраняя логику FSM.

## 4. Обработка "залипшей" кнопки пешехода

Следует ввести ограничение на длительность активного запроса (debouncing) и/или флаг, который сбрасывается после обработки одного перехода. Логика FSM изменится так, что повторные сигналы от кнопки будут игнорироваться до завершения текущего цикла светофора.
