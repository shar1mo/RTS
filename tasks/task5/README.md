...
## 1. Почему для `mlockall` могут требоваться права суперпользователя?

mlockall блокирует память процесса в RAM, что может исчерпать системные ресурсы. Без прав root процесс может получить ограничение на объём блокируемой памяти. Чтобы дать права без `sudo`, можно увеличить лимит `RLIMIT_MEMLOCK` через `ulimit -l` или настроить `/etc/security/limits.conf`.

## 2. Разница между `MCL_CURRENT` и `MCL_FUTURE`

* `MCL_CURRENT` блокирует уже выделенную память.
* `MCL_FUTURE` автоматически блокирует всю память, которая будет выделена после вызова. Использовать оба важно, чтобы заблокировать и существующую, и будущую память.

## 3. Главный недостаток реализованного пула памяти

Пул выделяет блоки фиксированного размера. Если потребуются блоки другого размера, нужно создавать отдельный пул, что неудобно и может привести к фрагментации.

## 4. Может ли `mlockall` защитить от всех источников задержек памяти?

Нет. `mlockall` предотвращает page faults, но задержки могут оставаться из-за кэша CPU, TLB, NUMA, или других аппаратных особенностей.
