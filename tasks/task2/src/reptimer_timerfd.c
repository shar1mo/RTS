 ...
 *   - Можно объединять несколько таймеров и файловых событий в одном epoll.
 *   - Нет сигналов (безопасно для многопоточности).
 *   - Работает в событийно-ориентированных приложениях (сетевые серверы, реакторы).
 * 
 * Сравнение с clock_nanosleep:
 *   - clock_nanosleep подходит для простых периодических циклов в одном потоке.
 *   - timerfd удобнее, когда таймеры должны "вписываться" в асинхронную модель:
 *     epoll/poll/select, без активного ожидания или блокирующих пауз.
 */
