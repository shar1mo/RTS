 ...
 * 
 * * Что такое scatter-gather I/O:
 *  - "Gather" (writev): собрать данные из нескольких несмежных областей памяти
 *    и записать их одним вызовом writev().
 *  - "Scatter" (readv): прочитать данные одним вызовом readv()
 *    и "раскидать" их по разным областям памяти.
 *
 * Почему это полезно:
 *   Меньше системных вызовов:
 *     Вместо 3 вызовов write() — один writev().
 *     Меньше переключений в ядро → выше производительность.
 *
 *   Нет лишнего копирования:
 *     Обычно приходится делать memcpy() в общий буфер перед отправкой.
 *     С writev() ядро само последовательно прочитает данные из нескольких
 *     буферов пользователя — без создания промежуточного буфера.
 *
 */
